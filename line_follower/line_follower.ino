...
void loop()
{
  ...
  int left_led_val = map(left_val, 0, 900, 0, 255);
  int right_led_val = map(right_val, 0, 900, 0, 255);
  analogWrite(LEFT_LED, left_led_val);
  analogWrite(RIGHT_LED, right_led_val);
...
void turn(int diff)
{
  if (diff < 0)
  {
    // Diverted to left
    // CW rotation
    left_pwm_delta = float(MAX_POWER - MID_POWER) * abs(diff) * 1.0 / MAX_DIFF;
    ...
    right_pwm_delta = float(MID_POWER - MIN_POWER) * abs(diff) * 1.0 / MAX_DIFF;
